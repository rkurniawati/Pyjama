...
public class SharedTest {
	...
	public static void tearDownAfterClass() throws Exception {
		...
		n.write(Data,"JsonResult","Shared","CompileJunitResults",false);
	}
	
	@Test
	public void PositiveTest1() {
		if(!positiveTest("PositiveTest1","shared_positive_test1","test1","Shared variables in prarllel region construct"))
			fail("Compile Failed");
	}
	
	@Test
	public void PositiveTest2() {
		if(!positiveTest("PositiveTest2","shared_positive_test2","test2","Shared variables in prarllel for construct"))
			fail("Compile Failed");
	}
	
	@Test
	public void PositiveTest3() {
		if(!positiveTest("PositiveTest3","shared_positive_test3","test3","Shared variables in prarllel section construct"))
			fail("Compile Failed");
	}
	
	@Test
	public void NegativeTest1() {		
		if(!negativeTest("NegativeTest1","shared_negative_test1","Shared variables in prarllel single construct"))
			fail("Compile Failed");
	}
	...
	private boolean positiveTest(String name,String filename,String test,String remark){
		...
		result.setCompilePJFilePath(SettingConstans.COMPILETARGETTESTCODEPATH+"/PyjamaCode/TestingClauses/Shared/"+filename+".pj");
		...
		try {
			PyjamaToJavaParser.parse(new File(SettingConstans.COMPILETARGETTESTCODEPATH+"/PyjamaCode/TestingClauses/Shared/"+filename+".pj"));
			JavaCompiler builder = new JavaCompiler();
			List<String> error =builder.buildJava(SettingConstans.COMPILETARGETTESTCODEPATH+"/PyjamaCode/TestingClauses/Shared/",test);
			if (error.size()>0){
