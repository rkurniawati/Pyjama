 ...
 * Threads will need to wait before executing their chunk of iterations 
 ...
 * 
 * The ordered directive must be within the dynamic extent of a for (OpenMP) or parallel for
 * construct with an ordered clause.
 * The ordered directive supports no Pyjama clauses.
 */
public class OrderedTest {
	...
	public static void tearDownAfterClass() throws Exception {
		...
		n.write(Data,"JsonResult","Ordered","CompileJunitResults",false);
	}
	
	@Test
	public void PositiveTest1() {
		if(!positiveTest("PositiveTest1","ordered_positive_test1","test1","In ordered area, code running as sequational program"))
			fail("Compile Failed");
	}
	
	@Test
	public void NegativeTest1() {		
		if(!negativeTest("NegativeTest1","ordered_negative_test1","No boundary in ordered region"))
			fail("Compile Failed");
	}
	...
	private boolean positiveTest(String name,String filename,String test,String remark){
		...
		result.setCompilePJFilePath(SettingConstans.COMPILETARGETTESTCODEPATH+"/PyjamaCode/TestingDirectives/Ordered/"+filename+".pj");
		...
		try {
			PyjamaToJavaParser.parse(new File(SettingConstans.COMPILETARGETTESTCODEPATH+"/PyjamaCode/TestingDirectives/Ordered/"+filename+".pj"));
			JavaCompiler builder = new JavaCompiler();
			List<String> error =builder.buildJava(SettingConstans.COMPILETARGETTESTCODEPATH+"/PyjamaCode/TestingDirectives/Ordered/",test);
			if (error.size()>0){
