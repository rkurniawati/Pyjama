...
public class Schedule_Group1_MacTest {
	...
	private void test(int threadNumber){
		...
		result.setTestName("parallel_for_static:"+arraySize);		
		result.setThreadCount(threadNumber);
		result.setArraySize(arraySize);
		
		try {
			List<MonitorInfoBean> results =test.parallel_for_static(threadNumber, arraySize);
