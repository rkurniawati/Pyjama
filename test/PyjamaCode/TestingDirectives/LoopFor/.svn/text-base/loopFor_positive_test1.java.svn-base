...
//#GEN#[-1]#PJ#
public class loopFor_positive_test1 {//#GEN#[-1]#PJ#
//#GEN#[-1]#PJ#
    /**
	 * Greater
	 * *///#GEN#[-1]#PJ#
    public int[] parallel_loopFor(int threadCount, int InitialValue, int EndValue, int Stride) {{//#GEN#[-1]#PJ#
        Pyjama.omp_set_num_threads(threadCount);//#GEN#[-1]#PJ#
        int[] array = new int[(Math.abs(InitialValue) + Math.abs(EndValue)) + 1];//#GEN#[-1]#PJ#
        int startIndex = 0;//#GEN#[-1]#PJ#
        if (InitialValue >= 0 && EndValue >= 0) startIndex = 0;//#GEN#[-1]#PJ#
        if (InitialValue < 0 && EndValue >= 0) startIndex = Math.abs(InitialValue);//#GEN#[-1]#PJ#
        if (InitialValue >= 0 && EndValue < 0) startIndex = Math.abs(EndValue);//#GEN#[-1]#PJ#
        if (InitialValue < 0 && EndValue < 0) startIndex = Math.max(Math.abs(InitialValue), Math.abs(EndValue));//#GEN#[-1]#PJ#
        ...
        class MyCallable implements Callable<ConcurrentHashMap<String,Object>> {
            ...
            public ConcurrentHashMap<String,Object> call() {
                ...
                {//#BEGIN firstprivate lastprivate reduction variables defined and initialized here
                    ...
                    if (((EndValue)-(InitialValue))%(Stride) == 0) {
                        OMP_end = OMP_end - 1;
                    }
