...
public class loopFor_Monitor{
	public List<MonitorInfoBean> parallel_for(int threadNumber,int n) throws Exception{
		...
		//#omp parallel for shared(n, completeNumber, result, array, service)
		for(int i = 0; i <n; i++){
			array[i]=i;
			completeNumber++;
			try {
