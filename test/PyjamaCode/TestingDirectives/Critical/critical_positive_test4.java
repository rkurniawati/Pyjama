package PyjamaCode.TestingDirectives.Critical;//#GEN#[-1]#PJ#
//#GEN#[-1]#PJ#
...
//#GEN#[-1]#PJ#
public class critical_positive_test4 {//#GEN#[-1]#PJ#
//#GEN#[-1]#PJ#
    public int parallel_for_critical(int threadNumber, int Number) {{//#GEN#[-1]#PJ#
        Pyjama.omp_set_num_threads(threadNumber);//#GEN#[-1]#PJ#
        int[] array = new int[Number + 1];//#GEN#[-1]#PJ#
        for (int i = 0; i <= Number; i++) array[i] = i;//#GEN#[-1]#PJ#
        int[] max = new int[] { array[0] };//#GEN#[-1]#PJ#
        ...
        inputlist__OMP_ParallelRegion_0.put("max",max);
        inputlist__OMP_ParallelRegion_0.put("array",array);
        inputlist__OMP_ParallelRegion_0.put("Number",Number);
...
class _OMP_ParallelRegion_0{
        ...
        public _OMP_ParallelRegion_0(int thread_num, InternalControlVariables icv, ConcurrentHashMap<String, Object> inputlist, ConcurrentHashMap<String, Object> outputlist) {
            ...
            max = (int[])OMP_inputList.get("max");
            array = (int[])OMP_inputList.get("array");
            Number = (Integer)OMP_inputList.get("Number");
            //#END shared variables initialised here
        }

        private void updateOutputListForSharedVars() {
            //BEGIN update outputlist
            OMP_outputList.put("max",max);
            OMP_outputList.put("array",array);
            OMP_outputList.put("Number",Number);
            ...
            public ConcurrentHashMap<String,Object> call() {
                ...
                {//#BEGIN firstprivate lastprivate reduction variables defined and initialized here
                    ...
                    for (OMP_local_iterator=OMP_Chunk_Starting_point; OMP_local_iterator<OMP_Chunk_Starting_point+OMP_Default_chunkSize_autoGenerated && OMP_Default_chunkSize_autoGenerated>0; ++OMP_local_iterator) {
                        ...
                        {//#GEN#[-1]#PJ#
                            if (array[i] > max[0]) {//#GEN#[-1]#PJ#
                                PjRuntime.OMP_lock.lock();//#GEN#[-1]#PJ#
                                try {//#GEN#[-1]#PJ#
                                    {//#GEN#[-1]#PJ#
                                        if (array[i] > max[0]) max[0] = array[i];//#GEN#[-1]#PJ#
                                    }} finally {//#GEN#[-1]#PJ#
