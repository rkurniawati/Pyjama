...
public class schedule_positive_test2 {
	...
	public int[] parallel_for_schedule(int threadNumber, int arraySize) {
		...
		//#omp parallel for shared(array, arraySize, threadNumber) schedule(dynamic)
	...
	public int[] parallel_for_schedule(int threadNumber, int arraySize,
		...
		//#omp parallel for shared(array, arraySize,threadNumber) private(chunkSize) schedule(dynamic,chunkSize)
		for (int i = 0; i < arraySize; i++) {
			int threadId = Pyjama.omp_get_thread_num();
			//#omp critical
			{
